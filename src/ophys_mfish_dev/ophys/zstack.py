...
def register_cortical_stack(zstack_path: Union[Path, str],
    ...
    stack_metadata : dict, optional
        Metadata for the stack, in case testing tifffile not generated
        by Scanimage. Should have keys: 'plane_order', 'num_slices', 'num_volumes',
    ...
    # 3A. Rererence channel
    if num_channels > 1:
        #stack_ch = np.squeeze(stack[:, ref_channel, :, :])
        # deinterleave the 0 axis
        stack_ch = stack[ref_channel::num_channels, :, :]
    else:
        stack_ch = stack
        ref_channel = 0

    reg_dict_ref = get_zstack_reg(stack_ch, plane_order, n_planes,
    ...
    if num_channels > 1:
        ...
        #stack_ch = np.squeeze(stack[:, target_channel, :, :])
        stack_ch = stack[target_channel::num_channels, :, :]
        reg_dict_target = get_zstack_reg_using_shifts(stack_ch, plane_order, n_planes,
                                                   n_repeats_per_plane, reg_dict_ref['shifts_within'], 
                                                   reg_dict_ref['shifts_between'], target_channel)
    ...
    if save:
        for i, d in enumerate(reg_dicts):
            ...
            out1 = save_registered_stack(plane_reg_stack, zstack_path, reg1_output_path, n_reg_steps=1)
